...
## Docker operations
There are two user-visible components to a Docker package: an `image`, which is
a collection of software, and `containers`, where the software runs. In this
...
`edgyr/container-run-scripts`. I'm going to assume the host is a Jetson Nano
called "Nano".
...
we could have created an isolated network that only the container and other
containers we specified could use.

### Stopping and restarting the container
Suppose we want to stop the container but keep all the files on its filesystem
intact. We do that by *stopping* the container. There's a script for that -
`stop-edgyr`:
