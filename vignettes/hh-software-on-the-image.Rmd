...
A Docker image is composed of layers. The first layer on `edgyr-ml` is the
NVIDIA `l4t-ml` image. ^[@NVIDIA2019d]
...
process adds a non-root user `edgyr` with `sudo` privileges. This is standard
practice for services built on Ubuntu Linux. `edgyr` has numeric user ID 1000
and group ID 1000; this will be important if you mount files from your Jetson
host home directory into an `edgyr` container.
...
`--system-site-packages` option. Then it adds the Python package `SymPy` 
...
- knitr (R Markdown)
- miniUI (required by `pkgdown` rebuild addin)
- pkgdown (package site builder)
- rmarkdown (R Markdown)
- rTorch (interface to PyTorch)
- tinytex (LaTeX / PDF generation)

Some notes about `tinytex`: it's installed, and it works. The full LaTeX on an
Ubuntu distribution is something like 4 gigabytes, and the image is already
almost 7 gigabytes, so this is a big deal.
